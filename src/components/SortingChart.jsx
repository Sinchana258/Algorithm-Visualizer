// src/components/SortingChart.jsx
...
function SortingChart() {
    ...
    // local paused state for rendering, because provider exposes pausedRef.current snapshot
    const [isPaused, setIsPaused] = useState(false);

    useEffect(() => {
        generateSortingArray();
        // sync local paused if provider paused changes (best-effort snapshot)
        setIsPaused(paused);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);

    // small helpers to call provider and sync UI
    const handlePause = () => {
        pauseSorting();
        setIsPaused(true);
    };
        ...
        <div className="mt-4 mb-4 flex flex-col items-center">
            ...
            <div className="max-w-3xl w-full">
                ...
                <div className="flex items-center gap-4 max-w-3xl mb-8">
                    ...
                    <button disabled={!sortingState.sorting || isPaused} onClick={handlePause} className="px-4 py-2 push-btn text-white-light">
                        Pause
                    </button>
                    <button disabled={!sortingState.sorting || !isPaused} onClick={handleResume} className="px-4 py-2 push-btn text-white-light">
                        Resume
                    </button>
                    <button disabled={!sortingState.sorting} onClick={handleStop} className="px-4 py-2 push-btn text-white-light">
                        Stop
                    </button>
